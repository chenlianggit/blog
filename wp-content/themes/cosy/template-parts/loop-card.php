...
================================
  	...
  	@Author: Dami
  	@Date:   2017-09-05 20:04:46
  	@Last Modified by:   Dami
  	@Last Modified time: 2017-11-13 14:02:42
...
if( is_category() ){
	...
	$term_data = wp_parse_args( (array) $term_data, array( 
            ...
            'cat_meta_show'               => true,
            'cat_insert_ad_card'          => false,
            'cat_insert_ad_card_text'     => true,
            'cat_insert_ad_card_position' => 3,
            'cat_insert_ad_card_url'      => 'http://www.suxing.me/',
            'cat_insert_ad_card_img'      => '',
            'cat_insert_ad_card_img_mobile' => '',
            'cat_insert_ad_card_html_pc' => '',
            'cat_insert_ad_card_html_mobile'    => '',
		) 
	);
    $thumpic = wp_get_attachment_image_src($term_data['cat_insert_ad_card_img'],'full');
    $thumpic_moblie = wp_get_attachment_image_src($term_data['cat_insert_ad_card_img_mobile'],'full');
    $ad_type = $term_data['cat_insert_ad_card'];
    if( !$paged ) :
    if( $ad_type == 'image'  &&  $term_data['cat_insert_ad_card_position'] == $GLOBALS['mii'] ){ ?>
        <div class="col-grid adv-grid col-xs-12 col-sm-4 col-md-4">
            <article class="post post-grid suxing-adv">
                <div class="visible-xs">
                    <a rel="nofollow"  href="<?php echo $term_data['cat_insert_ad_card_url']; ?>">
                        <img <?php echo is_lazysizes(); ?>src="<?php echo $thumpic_moblie[0]; ?>">
                    </a>
                      <?php
                          if( $term_data['cat_insert_ad_card_text'] ){
                              echo '<span>广告</span>';
                          }
                      ?>
                </div>
    ...
    if( $ad_type == 'code'  && $term_data['cat_insert_ad_card_position'] == $GLOBALS['mii']  ){ ?>
      <div class="col-grid adv-grid col-xs-12 col-sm-4 col-md-4">
        <article class="post post-grid suxing-adv">
            <div class="visible-xs">
                <?php echo $term_data['cat_insert_ad_card_html_mobile']; ?>
            </div>
            <div class="visible-sm visible-md visible-lg">
                <?php echo $term_data['cat_insert_ad_card_html_pc']; ?>
            </div>
        </article>
      </div>
    <?php }
    ...
    <article class="post post-grid">
        <div class="image pull-left">
        	<a href="<?php the_permalink(); ?>">
        		<?php if( $term_data['cat_layout'] == 'card-4' ){ ?>
